...
- **Handoff / Próximos Passos:** servidor e APK implementados (pytest 178, web 27, Android 33). Roteiro em `.../cv3-ds1-us5-um-vinculo-por-vez/test-guide.md`; APK 0.10.0 local SHA-256 `594a09a4…9cd1` (telas refeitas após o 1º teste físico). Observação em aberto: no 1º teste, o botão "Passar controle" não apareceu na quadra q1 (apareceu na q2); não reproduzido, Navigator vai retestar. Site não mudou nesta HU. Próximo: Navigator valida no Watch real com a branch no Mini PC; depois Checkpoint 3 (revisão).
